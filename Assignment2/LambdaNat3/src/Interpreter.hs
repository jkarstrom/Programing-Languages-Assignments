...
evalCBN (EIf e1 e2 e3 e4) = if (evalCBN e1) == (evalCBN e2) then evalCBN e3 else evalCBN e4 -- if-then-else B M N = B M N
evalCBN (ELet i e1 e2) = evalCBN (EApp (EAbs i e2) e1)  --let x = e1 in e2
evalCBN (EMinusOne e) = case (evalCBN e) of --if n matches ENatS m then m is one less than n (pattern matching)
    ENat0 -> ENat0
    (ENatS e) -> e
--    (ENatS e1) -> EvalCBN (e1)
--    (EMinusOne e2) -> evalCBN (e2)
-- extra points
evalCBN (ERec i e1 e2) = evalCBN (EApp (EAbs i e2) (EFix (EAbs i e1))) --recursion let
...
-- subst id s (EMinusOne e) = EMinusOne (subst id s e1) (subst id s e2)
-- extra points
subst id s (ERec i e1 e2) = ERec i (subst id s e1) (subst id s e2)
